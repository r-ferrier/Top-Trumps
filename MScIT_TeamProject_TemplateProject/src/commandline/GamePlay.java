...
public class GamePlay {
	...
	private boolean humanKnockedOut;
...
//		new TestLog(true); // remove from final version. this needs to be here to prevent log printing in console
	...
	public void announceRoundNumber() {
		int roundNumber = roundCounter + 1;
		System.out.println("Round " + roundNumber + ": Players have drawn their cards."); 
	}

	/**
	 * Once the user has entered a name they are added to an ArrayList of players.
	 * Players ArrayList is shuffled and cards dealt into player's 'hand'. User
	 * offered the option to begin game or view stats.
	 */
	private void gameBegins() {
		...
		while (startGame == false) {
			System.out.println("\nWould you like to start a new game, see previous game stats or quit? "
					+ " \nEnter: 1 for a NEW GAME, 0 for previous GAME STATS or 9 to QUIT.");
			try {
				...
				if (gameStats == 0) {
					...
					players.add(new Player(s.nextLine(), true, 0));
	 ...
	 * players ArrayList. Current player index is changed to reflect new position.
	 * Check if the human is still in the game.
	 */
	private void removeKnockedOutPlayers() {
		for (int i = 0; i < players.size(); i++) {
			...
			if (players.get(i).amIKnockedOut()) {
				...
				if (currentPlayer > i) {
					currentPlayer--; 
					// If current player's position was after i or if it was i and the current player
					// is being knocked out after a series of draws, current player position is
					// moved one position down in the ArrayList to reflect either their change in
					// position or the turn moving to the next player.
				}
	...
	/**
	 * Call this method each time a player is knocked out. Method checks the number
	 * of active players and sets gameOver to true when there is only one active
	 * player remaining.
	 */
	...
	/**
	 * When game is over, loop through players and return the player who has not
	 * been knocked out
	 * 
			...
			while (!categorySelected) {
				...
				try {
					...
					while (chosenCategory > 5 || chosenCategory < 1) {
						System.out.println(
								"I'm sorry, that is not a valid category! Please choose again, your number must be between"
										+ " 1 and 5.");
						chosenCategory = categorySelection.nextInt();
					}
	...
	/**
	 * Compares the value of the chosen category on each player's top card and finds
	 * the highest positive value. It checks if the round is draw by looking for two
	 * matching positive integers of the highest value. If the round is a draw, the
	 * winner is set to null and the method returns false. If it is not a draw, the
	 * currentPlayer index is updated to reflect the position of the round winner
	...
	private boolean declareRoundWinOrDraw() {
		...
		if (topCardValues.get(topCardValues.size() - 1) == topCardValues.get(topCardValues.size() - 2)) {
			...
			System.out
					.println(winner.getName() + " won this round with the card " + winner.getTopCard().getDescription()
	...
	/**
	 * If the round was won, loop through players and if player is not the winner,
	 * get their top card, remove it, and add it to the winners 'hand'. Add any
	 * cards from the communal pile to the winner's hand. If the round was a draw,
	 * get the top card from each player and add to the communal pile.
	 * 
